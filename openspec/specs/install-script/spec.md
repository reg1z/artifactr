...
Defines the behavior of the `install.sh` bash installer script for artifactr on Linux and macOS. The script handles environment detection, Python version gating, tiered installation (pipx preferred, managed venv fallback), PATH configuration, upgrade, and uninstall. It is fully external to the Python package — no changes to `src/artifactr/`.
