...
Spec for zip file input support in `art store`. Covers auto-detection of zip archive type (single artifact vs vault bundle), extraction to a temp directory, and mutual exclusion with `--global`.
