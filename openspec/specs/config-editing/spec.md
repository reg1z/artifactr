...
- **THEN** the global config file at `<config_dir>/config.yaml` MUST be opened in the user's preferred editor using the same resolution as `art edit` (`$VISUAL` → `$EDITOR` → nano/nvim/vim/vi → edit → notepad.exe on Windows)
